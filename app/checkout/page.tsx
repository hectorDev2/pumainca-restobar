...
export default function CheckoutPage() {
  const { cart, cartTotal: totals } = useCart();
    ...
    <div className="flex flex-col min-h-screen bg-background-dark">
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-zinc-800 bg-background-dark/95 backdrop-blur px-6 md:px-10 py-3">
        ...
        <div className="flex items-center gap-6">
          <Link href="/cart" className="text-text-secondary hover:text-white text-sm font-medium transition-colors hidden sm:block">Volver al Carrito</Link>
                  ...
                  {cart.map((item, i) => {
                     ...
                     return (
                      <div key={i} className="flex justify-between items-start">
                        ...
                        <span className="text-white font-medium text-sm">S./{(unitPrice * item.quantity).toFixed(2)}</span>
                      </div>
                    );
                  })}
                </div>
