...
self.addEventListener('fetch', (event) => {
  ...
  if (isImageRequest(url)) {
    ...
    event.respondWith(cacheFirst(request, CACHE_NAMES.images, 30 * 24 * 60 * 60 * 1000)); // 30 días
  } else if (isStaticResource(url)) {
    // ESTÁTICOS (JS, CSS, Fonts): Cache First
    event.respondWith(cacheFirst(request, CACHE_NAMES.static, 365 * 24 * 60 * 60 * 1000)); // 1 año
