...
const Sidebar: React.FC<Props> = ({ 
  ...
  const getIcon = (id: string) => {
    switch(id) {
      case 'platos-principales': return 'dinner_dining';
      case 'platos-vegetarianos': return 'spa';
      case 'pizzas': return 'local_pizza';
      case 'bebidas': return 'local_bar';
      case 'postres': return 'icecream';
    ...
    <aside className="w-64 bg-earth-950 border-r border-earth-700/30 flex-col hidden md:flex shrink-0 transition-all duration-300">
      <div className="p-6 space-y-10">
        ...
        <div className="space-y-4">
          <div className="flex flex-col">
            <h3 className="text-ember-600 text-sm font-bold uppercase tracking-widest mb-1">Menú</h3>
          ...
          <div className="flex flex-col gap-1 relative">
            {allCategories.map((cat) => (
              ...
              >
                ...
                <span className="text-sm font-bold relative z-20">{cat.name}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col">
            <h3 className="text-ember-600 text-sm font-bold uppercase tracking-widest mb-1">Dietética</h3>
            ...
            {['Vegetariano', 'Vegano', 'Sin Gluten'].map((label) => (
              <label key={label} className="flex items-center gap-3 cursor-pointer group">
                <input 
                  ...
                  aria-label={label}
                />
                <span className={`text-sm font-medium group-hover:text-cream transition-colors ${selectedDietaryFilters.includes(label) ? 'text-ember-600' : 'text-honey'}`}>
                  {label}
                </span>
              </label>
            ))}
      ...
      <div className="mt-auto p-6 border-t border-earth-700/30">
        <div className="flex items-center gap-3 p-2 hover:bg-earth-800/40 rounded-xl transition-colors cursor-pointer">
          <div className="size-10 rounded-full bg-cover bg-center border border-earth-700/50" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1599566150163-29194dcaad36?auto=format&fit=crop&w=100&q=80')` }}></div>
          <div className="flex flex-col">
            <span className="text-sm font-bold text-cream">Invitado</span>
            <span className="text-[10px] text-ember-600 font-bold uppercase tracking-wider">Registrarse</span>
          </div>
        </div>
      </div>
    </aside>
