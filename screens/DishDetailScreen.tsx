...
interface Props {
  ...
  cartCount: number;
  onAddToCart: (dish: Product, quantity: number, options: any) => void;
}

const DishDetailScreen: React.FC<Props> = ({ dish, onNavigate, cartCount, onAddToCart }) => {
  const [quantity, setQuantity] = useState(1);
  const [activeImage, setActiveImage] = useState(dish.image);
  const [cookingPoint, setCookingPoint] = useState('medium-rare');
  
  // Handle Variable Pricing (Size Selection)
  // If price is object, default to first key
  ...
  const [selectedSize, setSelectedSize] = useState<string | undefined>(
    hasMultipleSizes ? sizeOptions[0] : undefined
  );
  ...
  // Update active image if dish changes
  useEffect(() => {
  ...
  const handleAddToCart = () => {
    onAddToCart(dish, quantity, { cookingPoint, selectedSize });
    onNavigate('menu');
  };
    ...
    <div className="flex flex-col min-h-screen bg-background-dark overflow-x-hidden">
      ...
      <main className="flex-1 max-w-[1280px] mx-auto w-full px-4 lg:px-10 py-6">
        <div className="flex flex-wrap gap-2 py-4 mb-4 text-sm font-medium text-text-secondary">
          <button onClick={() => onNavigate('home')} className="hover:text-primary transition-colors">Inicio</button>
          <span>/</span>
          <button onClick={() => onNavigate('menu')} className="hover:text-primary transition-colors">Menú</button>
          <span>/</span>
        ...
        <section className="border-t border-zinc-800 pt-16 pb-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {relatedDishes.map((d) => {
              ...
              return (
                <div key={d.id} className="group cursor-pointer" onClick={() => onNavigate('dish-detail', d.id)}>
